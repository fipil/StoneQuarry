...
namespace StoneQuarry.Lib.Config
{
    internal static class ConfigUtil
    {
        public static object LoadConfig(ICoreAPI api, Type type, ref object config, out int loadedVersion, ILogger logger)
        {
            ...
            var jsonConfig = LoadOrCreateConfig(api, configAttr.Filename, logger ?? api.Logger, new Dictionary<string, JsonConfigValue<object>>()) ?? [];
        ...
        private static T LoadOrCreateConfig<T>(ICoreAPI api, string file, ILogger logger, T? defaultConfig = null) where T : class, new()
        {
